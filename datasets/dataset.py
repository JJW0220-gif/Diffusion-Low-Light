...
def _resolve_split_root(config, split: str) -> Path:
    root = Path(config.data.data_dir)
    candidate = root / split
    if candidate.is_dir():
        return candidate

    dataset_name = getattr(config.data, "train_dataset" if split == "train" else "val_dataset", "")
    if dataset_name:
        candidate = root / dataset_name / split
        if candidate.is_dir():
            return candidate
...
class _CompatPairedDataset(Dataset):
    def __init__(self, root: Union[str, Path], transform: Transform = None):
        self.dataset = PairedLowLightDataset(root, transform=transform)

    def __len__(self) -> int:
        return len(self.dataset)

    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, str]:
        stem = self.dataset.stems[idx]
        input_tensor, gt_tensor = self.dataset[idx]
        return torch.cat([input_tensor, gt_tensor], dim=0), stem
...
class LLdataset:
    ...
    def get_loaders(self, parse_patches=True):
        ...
        val_dataset = _CompatPairedDataset(_resolve_split_root(self.config, "val"), transform=None)
